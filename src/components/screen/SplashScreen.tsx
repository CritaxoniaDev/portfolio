...
export const SplashScreen = ({ onComplete, minDuration = 2800 }: Props) => {
    ...
    return (
        <div
            className="fixed inset-0 z-[9999] flex flex-col overflow-hidden bg-[rgb(244,243,238)]"
            style={{
        ...
        >
            ...
            {/* Top meta row */}
            <div
                className="sp-fade absolute left-5 right-5 top-6 z-10 flex items-center justify-between gap-4 text-[0.68rem] leading-none text-black/55 smallcaps sm:left-6 sm:right-6 sm:top-8 md:left-12 md:right-12 md:top-10"
            ...
            {/* Corner brackets */}
            <span className="sp-bracket absolute top-20 left-5 h-4 w-4 border-t border-l border-black sm:top-24 sm:left-6 md:top-28 md:left-12" />
            <span className="sp-bracket absolute top-20 right-5 h-4 w-4 border-t border-r border-black sm:top-24 sm:right-6 md:top-28 md:right-12" />
            <span className="sp-bracket absolute bottom-32 left-5 h-4 w-4 border-b border-l border-black sm:bottom-36 sm:left-6 md:bottom-40 md:left-12" />
            <span className="sp-bracket absolute bottom-32 right-5 h-4 w-4 border-b border-r border-black sm:bottom-36 sm:right-6 md:bottom-40 md:right-12" />
            ...
            >
                <svg width="44" height="44" viewBox="0 0 44 44" className="sp-spin opacity-60" aria-hidden="true">
                    <circle cx="22" cy="22" r="18" fill="none" stroke="#0a0a0a" strokeWidth="0.8" strokeDasharray="2 4" />
                    <circle cx="22" cy="22" r="2" fill="#0a0a0a" />
                    <line x1="22" y1="4" x2="22" y2="10" stroke="#0a0a0a" strokeWidth="0.8" />
                </svg>
            </div>
            ...
            <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-5 sm:px-6 md:px-12">
                <p
                    className="sp-fade mb-6 flex items-center gap-3 text-[0.72rem] leading-none text-black/65 smallcaps sm:mb-8 sm:text-[0.76rem]"
                    style={{ animationDelay: '0.2s' }}
                >
                    <span className="block h-px w-8 bg-black/40 sm:w-10" />
                    <span>A Portfolio</span>
                    <span className="font-mono text-black/40">— Vol. 005</span>
                </p>
                ...
                <p
                    className="sp-fade font-serif-alt mt-6 text-center text-[1.2rem] italic text-black/60 sm:mt-8 sm:text-[1.4rem]"
                ...
                >
                    <span className="block h-px w-12 bg-black/25" />
            ...
            <div className="relative z-10 px-5 pb-10 sm:px-6 sm:pb-12 md:px-12 md:pb-16">
                ...
                >
                    ...
                    <div className="min-w-0">
                        <div className="mb-2 font-mono text-[0.55rem] leading-none tracking-[0.22em] text-black/40 smallcaps">
                            Status
                        </div>
                        {phase === 'loading' ? (
                            <div
                                key={statusIdx}
                                className="sp-status font-serif-alt truncate text-[1.1rem] italic text-black/75 sm:text-[1.3rem]"
                            >
                                {statuses[statusIdx]}
                                <span className="text-black/40">...</span>
                            </div>
                        ) : (
                            <div className="sp-ready font-serif-alt text-[1.1rem] italic text-black sm:text-[1.3rem]">
                                Ready
                                <span className="ml-2 inline-block h-2 w-2 -translate-y-[3px] bg-black" />
                            </div>
                        )}
                    </div>
                ...
                {/* Progress bar */}
                <div
                    className="sp-fade relative h-px w-full bg-black/15"
                ...
                >
                    ...
                    <div
                        className="absolute -top-[3px] h-[7px] w-px bg-black"
                        style={{
                            left: `${progress}%`,
                    ...
                    <div className="pointer-events-none absolute inset-0">
                        {[25, 50, 75].map((pct) => (
                            <span
                                key={pct}
                                className="absolute top-1/2 h-1 w-px -translate-y-1/2 bg-black/30"
                                style={{ left: `${pct}%` }}
                            />
                        ))}
                    </div>
                ...
                {/* Bottom meta */}
                <div
                    className="sp-fade mt-4 flex items-center justify-between font-mono text-[0.62rem] tracking-[0.18em] text-black/40 smallcaps"
